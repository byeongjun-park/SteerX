...
class Record3dLoader_Customized:
    ...
    def __init__(self, data_dir: Path, conf_threshold: float = 1.0, foreground_conf_threshold: float = 0.1, xyzw=True):
...
class Record3dFrame:
    ...
    def get_point_cloud(
        ...
        # Foreground points
        homo_grid = np.pad(grid[fg_conf_mask], ((0, 0), (0, 1)), constant_values=1)
        local_dirs = np.einsum("ij,bj->bi", np.linalg.inv(K), homo_grid)
        dirs = np.einsum("ij,bj->bi", T_world_camera[:3, :3], local_dirs)
        points = (T_world_camera[:3, 3] + dirs * depth[fg_conf_mask, None]).astype(np.float32)
        point_colors = rgb[fg_conf_mask]
