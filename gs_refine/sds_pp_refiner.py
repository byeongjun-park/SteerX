...
from gs_refine.gs_util import GaussianRenderer, GaussiansManeger
...
class GSRefinerSDSPlusPlus(nn.Module):
    def __init__(
        ...
        pipe = StableDiffusionPipeline.from_pretrained(sd_model_key, cache_dir=".cache/models")
    ...
    def train_step(
        ...
        if self.use_lods:
            with torch.enable_grad():
                noise_pred_learnable = self.unet(
                    latents_noisy, t, encoder_hidden_states=learnable_text_embeddings
                ).sample
        ...
        loss_latent_sds = (
            F.mse_loss(latents, latents_pred, reduction="none").sum([1, 2, 3]) * w * alpha / sigma
        ).sum() / B
        loss_image_sds = (
            F.mse_loss(images, images_pred, reduction="none").sum([1, 2, 3]) * w * alpha / sigma
        ).sum() / B
    ...
    def refine_gaussians(self, gaussians, text, dense_cameras):
        ...
        new_gaussians = GaussiansManeger(
            xyz_original, features_original, opacity_original, scales_original, rotations_original, self.lrs
        )
        ...
        for i in tqdm.trange(self.total_iterations, desc="Refining..."):
            if i % self.densification_interval == 0 and i != 0:
                new_gaussians.densify_and_prune(opacity_threshold=self.opacity_threshold)
            ...
            with torch.amp.autocast("cuda", enabled=False):
                ...
                with torch.no_grad():
                    images_original = self.renderer(cameras, gaussians_original, h=self.img_size, w=self.img_size)[0]

                gaussians = new_gaussians()
                images_pred= self.renderer(cameras, gaussians, h=self.img_size, w=self.img_size)[0]

            anneal_t = int((i / self.total_iterations) ** (1 / 2) * (self.min_step - self.max_step) + self.max_step)
            t = torch.full((1,), anneal_t, dtype=torch.long, device=self.device)

            loss_latent_sds, loss_img_sds, loss_embedding = self.train_step(
                images_pred.squeeze(0), t, text_embeddings, uncond_text_embeddings, learnable_text_embeddings
            )

            image_variation_loss = F.mse_loss(images_original, images_pred, reduction='sum') / self.num_views

            loss = (
                    loss_latent_sds * self.lambda_latent_sds +
                    loss_img_sds * self.lambda_image_sds +
                    image_variation_loss * self.lambda_image_variation +
                    loss_embedding
            )
            ...
            big_points_ws = scales.max(dim=1).values > 0.05
            ...
            for radii, viewspace_points in zip(self.renderer.radii, self.renderer.viewspace_points):
                ...
                new_gaussians.max_radii2D[visibility_filter] = torch.max(
                    new_gaussians.max_radii2D[visibility_filter], radii[visibility_filter]
                )
