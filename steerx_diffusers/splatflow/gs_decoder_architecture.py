...
from steerx_diffusers.splatflow.utils.decoder_splatting import DecoderSplatting
...
class GSDecoder(nn.Module):
    ...
    def forward(self, latents):
        latent_image, latent_depth, latent_ray = latents.split([16, 16, 6], dim=2)
        extrinsics, intrinsics, _ = optimize_plucker_ray(latent_ray)

        # calculate plucker rays
        lh, lw = latent_image.shape[-2:]
        ray_embeddings = get_plucker_rays(extrinsics[:, :, :3], intrinsics, h=lh, w=lw, stride=8)

        x = torch.cat([latent_image, latent_depth, ray_embeddings], dim=2)  # channels: 16 + 16 + 9 = 41
        ...
        with torch.amp.autocast("cuda", enabled=False):
            gs = self.splatter(x.float(), extrinsics[:, :, :3], intrinsics)

            return gs, intrinsics[0], torch.linalg.inv(extrinsics[0])
