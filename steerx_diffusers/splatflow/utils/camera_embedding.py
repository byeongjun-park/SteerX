...
def optimize_plucker_ray(ray_latent):
    ...
    averaged_f = new_intrinsics[..., [0, 1], [0, 1]].mean()  # Modify to set fx == fy following MV-DUST3R+
    ff = nn.Parameter(averaged_f[None], requires_grad=True)
    ...
    for i in range(num_iterations + 1):
        optimizer.zero_grad()
        scale_matrix = torch.diag(torch.cat([ff, ff, torch.ones_like(ff[:1])], dim=0))
