...
@torch.no_grad()
def inference(pairs, model, device, batch_size=8, verbose=True, use_amp=True):
    if verbose:
    ...
    for i in tqdm.trange(0, len(pairs), batch_size, disable=not verbose):
        ...
        res = loss_of_one_batch(collate_with_cat(pairs[i:i+batch_size]), model, None, device, use_amp=use_amp)
